...
Embark = {
  ...
  deployContracts: function(_web3, env, contractsConfigFile, contractFiles, destFile, chainFile, withProvider, withChain, cb) {
    var gasPrice = this.blockchainConfig.config().gasPrice;
    var gasLimit = this.blockchainConfig.config().gasLimit;
    this.contractsConfig  = (new Config.Contracts(gasPrice, gasLimit, contractFiles, env));
    this.contractsConfig.loadConfigFile(contractsConfigFile);

    if (_web3 !== undefined && _web3 !== null) {
      this.web3 = _web3;
    } else {
      this.web3 = new Web3();

      var rpcHost = this.blockchainConfig.config().rpcHost;
      var rpcPort = this.blockchainConfig.config().rpcPort;

      try {
        this.web3.setProvider(new this.web3.providers.HttpProvider("http://" + rpcHost + ":" + rpcPort));
        primaryAddress = this.web3.eth.coinbase;
        this.web3.eth.defaultAccount = primaryAddress;
      } catch (e) {
        throw new Error("==== can't connect to " + rpcHost + ":" + rpcPort + " check if an ethereum node is running");
      }
    }
    ...
    var deploy = new Deploy(contractFiles, this.blockchainConfig.config(), this.contractsConfig, this.chainManager, withChain, this.web3);
    deploy.deploy_contracts(function() {
      console.log("contracts deployed; generating abi file");
      var result = ""
      if (withProvider) {
        result += deploy.generate_provider_file(rpcHost, rpcPort);
      }
